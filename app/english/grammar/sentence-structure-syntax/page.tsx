...
type GrammarSentenceStructureSyntaxTest = {
  id: number
  title: string
...
type GrammarSentenceStructureSyntaxQuestion = {
  id: number
  test_id: number
}

type GrammarSentenceStructureSyntaxProgress = {
  id: string
  user_id: string
  test_id: number | null
...
type TestWithProgress = GrammarSentenceStructureSyntaxTest & {
  score: number
...
export default function SentenceStructureSyntaxPage() {
  ...
  useEffect(() => {
    ...
    const raw = localStorage.getItem("sentence_structure_syntax_review_ids")
    if (!raw) {
      setReviewIds([])
      return
    }
  ...
  async function fetchTests() {
    ...
    const { data: testsData, error: testsError } = await supabase
      .from("grammar_sentence_structure_syntax_tests")
      .select("*")
      .order("created_at", { ascending: false })

    if (testsError) {
      console.error("Error loading sentence structure syntax tests:", testsError)
      setLoading(false)
      return
    }

    let allTests = (testsData || []) as GrammarSentenceStructureSyntaxTest[]
    ...
    if (mode === "review") {
      ...
      if (reviewQuestionsError) {
        console.error(
          "Error loading sentence structure syntax review questions:",
      ...
      const reviewQuestions =
        (reviewQuestionsData || []) as GrammarSentenceStructureSyntaxQuestion[]
    ...
    if (progressError) {
      console.error("Error loading sentence structure syntax progress:", progressError)
    ...
    const progressRows = (progressData || []) as GrammarSentenceStructureSyntaxProgress[]
    const latestProgressMap = new Map<number, GrammarSentenceStructureSyntaxProgress>()
      ...
      <>
        ...
        <p style={styles.message}>
          {mode === "review"
            ...
            : "Loading Sentence Structure & Syntax tests..."}
        </p>
      </>
    )
  }
          ...
          <div style={styles.heroCard}>
            ...
            <p style={styles.subtitle}>
              {mode === "review"
                ...
                : "Choose a Sentence Structure & Syntax test and answer 10 multiple-choice questions."}
            </p>
              ...
              {filteredTests.length === 0 ? (
                ...
                <div style={styles.grid}>
                  {filteredTests.map((test) => {
                    const href =
                      mode === "review"
                        ? `/english/grammar/sentence-structure-syntax/${test.id}?mode=review`
                        : `/english/grammar/sentence-structure-syntax/${test.id}`
                      ...
                      >
                        ...
                        <p style={styles.preview}>
                          {test.description?.trim()
                            ? test.description
                            : "Practise clauses, phrases, word order, sentence types, punctuation sense, and sentence structure in this test."}
                        </p>

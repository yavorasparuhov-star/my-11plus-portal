...
function PrimaryWordClassesContent() {
  ...
  async function fetchTests() {
    ...
    if (testsError) {
      console.error("Error loading primary word classes tests:", testsError)
      setLoading(false)
      return
    }
    ...
    if (progressError) {
      console.error("Error loading primary word classes progress:", progressError)
...
function getScorePercentage(score: number, isCompleted: boolean) {
  ...
  function getCompletedDateContent(test: TestWithProgress) {
    ...
    const completedDate = new Date(test.completed_at).toLocaleDateString(
      "en-GB",
      {
        day: "2-digit",
        month: "short",
            ...
            <div style={styles.accessInfo}>
              {plan === "guest"
                ...
                : plan === "free"
                  ...
                  : "Your membership unlocks all Primary Word Classes tests."}
            </div>

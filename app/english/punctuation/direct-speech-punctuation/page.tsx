...
type PunctuationDirectSpeechPunctuationTest = {
  id: number
  title: string
...
type PunctuationDirectSpeechPunctuationQuestion = {
  id: number
  test_id: number
}

type PunctuationDirectSpeechPunctuationProgress = {
  id: string
  user_id: string
  test_id: number | null
...
type TestWithProgress = PunctuationDirectSpeechPunctuationTest & {
  score: number
...
export default function DirectSpeechPunctuationPage() {
  ...
  useEffect(() => {
    ...
    const raw = localStorage.getItem("direct_speech_punctuation_review_ids")
    if (!raw) {
      setReviewIds([])
      return
    }
  ...
  async function fetchTests() {
    ...
    const { data: testsData, error: testsError } = await supabase
      .from("punctuation_direct_speech_punctuation_tests")
      .select("*")
      .order("created_at", { ascending: false })

    if (testsError) {
      console.error("Error loading direct speech punctuation tests:", testsError)
      setLoading(false)
      return
    }

    let allTests = (testsData || []) as PunctuationDirectSpeechPunctuationTest[]
    ...
    if (mode === "review") {
      ...
      if (reviewQuestionsError) {
        console.error(
          "Error loading direct speech punctuation review questions:",
      ...
      const reviewQuestions =
        (reviewQuestionsData || []) as PunctuationDirectSpeechPunctuationQuestion[]
    ...
    if (progressError) {
      console.error("Error loading direct speech punctuation progress:", progressError)
    ...
    const progressRows =
      (progressData || []) as PunctuationDirectSpeechPunctuationProgress[]
    const latestProgressMap = new Map<number, PunctuationDirectSpeechPunctuationProgress>()
      ...
      <>
        ...
        <p style={styles.message}>
          {mode === "review"
            ...
            : "Loading Direct Speech Punctuation tests..."}
        </p>
      </>
    )
  }
        ...
        <div style={styles.container}>
          <div style={styles.heroCard}>
            ...
            <p style={styles.subtitle}>
              {mode === "review"
                ? "Revise your saved punctuation mistakes and strengthen direct speech punctuation skills."
                : "Choose a Direct Speech Punctuation test and answer 10 multiple-choice questions."}
            </p>
          ...
          {tests.length === 0 ? (
            <div style={styles.emptyCard}>
              <h2>
                {mode === "review"
                  ? "No Direct Speech Punctuation review items found"
                  : "No Direct Speech Punctuation tests yet"}
              </h2>
              <p>
                {mode === "review"
              ...
              {filteredTests.length === 0 ? (
                ...
                <div style={styles.grid}>
                  {filteredTests.map((test) => {
                    const href =
                      mode === "review"
                        ? `/english/punctuation/direct-speech-punctuation/${test.id}?mode=review`
                        : `/english/punctuation/direct-speech-punctuation/${test.id}`
                      ...
                      >
                        ...
                        <p style={styles.preview}>
                          {test.description?.trim()
                            ? test.description
                            : "Practise speech marks, commas, capital letters, reporting clauses, and punctuation around spoken words in this test."}
                        </p>

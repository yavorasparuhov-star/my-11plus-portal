...
type PunctuationApostrophesTest = {
  id: number
  title: string
...
type PunctuationApostrophesQuestion = {
  id: number
  test_id: number
}

type PunctuationApostrophesProgress = {
  id: string
  user_id: string
  test_id: number | null
...
type TestWithProgress = PunctuationApostrophesTest & {
  score: number
...
export default function ApostrophesPage() {
  ...
  useEffect(() => {
    ...
    const raw = localStorage.getItem("apostrophes_review_ids")
    if (!raw) {
      setReviewIds([])
      return
    }
  ...
  async function fetchTests() {
    ...
    const { data: testsData, error: testsError } = await supabase
      .from("punctuation_apostrophes_tests")
      .select("*")
    ...
    let allTests = (testsData || []) as PunctuationApostrophesTest[]
    ...
    if (mode === "review") {
      ...
      const reviewQuestions = (reviewQuestionsData || []) as PunctuationApostrophesQuestion[]
    ...
    const progressRows = (progressData || []) as PunctuationApostrophesProgress[]
    const latestProgressMap = new Map<number, PunctuationApostrophesProgress>()

...
export default function VRSequencePatternsPage() {
  ...
  async function loadTests() {
    ...
    if (error) {
      console.error("Error loading sequence & patterns tests:", error)
      setTests([])
      setLoading(false)
      return
    }
    ...
    if (resultError) {
      console.error("Error loading latest VR sequence pattern results:", resultError)
  ...
  function getDifficultyBadgeStyle(level: number | null): CSSProperties {
  ...
  function getCompletedDateContent(test: TestWithProgress) {
    ...
    return (
      <Link
        href={`/results/vr/sequence-pattern/${test.id}`}
  ...
  function getTestButton(test: TestWithProgress) {
    const href = `/vr/sequence-patterns/${test.id}`
  ...
  if (loading) {
    return (
      <>
        <Header />
        <p style={styles.message}>Loading sequence & patterns tests...</p>
      </>
    )
  }

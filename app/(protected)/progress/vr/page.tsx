...
type VRBenchmarkRpcRow = {
  difficulty: number
  average_success: number
}

type CustomVRAttemptRow = {
  id: string
...
function normaliseCustomVRCategory(
...
function getCustomVRItemCategory(
  item: CustomVRAttemptItemRow
): Exclude<CategoryFilter, "all"> | null {
  return (
    normaliseCustomVRCategory(item.subtopic_key) ??
    normaliseCustomVRCategory(
    ...
    normaliseCustomVRCategory(item.topic_key) ??
    normaliseCustomVRCategory(
      getSnapshotString(item.question_snapshot, [
        "topicKey",
        "topic_key",
...
function buildCustomVRProgressRows(
  attempts: CustomVRAttemptRow[],
  items: CustomVRAttemptItemRow[]
): EnrichedVRProgressRow[] {
  const itemsByAttempt = items.reduce((acc, item) => {
    ...
    return acc
  }, {} as Record<string, CustomVRAttemptItemRow[]>)
  ...
  return attempts.flatMap((attempt) => {
    ...
    const groups = attemptItems.reduce((acc, item) => {
      const category = getCustomVRItemCategory(item)
      ...
      return acc
    }, {} as Partial<Record<Exclude<CategoryFilter, "all">, CustomVRAttemptItemRow[]>>)
    ...
    return Object.entries(groups).flatMap(([category, groupedItems]) => {
      ...
      return [
        {
          id: `vr-custom-${attempt.id}-${category}`,
          user_id: "",
          test_id: null,
...
export default function VRProgressPage() {
  ...
  const [benchmarkData, setBenchmarkData] =
    useState<VRBenchmarkRpcRow | null>(null)
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      if (progressError) {
        console.error("Error loading VR progress:", {
      ...
      if (testIds.length > 0) {
        ...
        if (testsError) {
          console.error("Error loading VR tests:", {
      ...
      if (customAttemptsError) {
        console.error("Error loading custom VR attempts:", {
      ...
      const customAttemptRows =
        (customAttemptsData ?? []) as CustomVRAttemptRow[]
      const customAttemptIds = customAttemptRows.map((attempt) => attempt.id)

      let customAttemptItemRows: CustomVRAttemptItemRow[] = []
      ...
      if (customAttemptIds.length > 0) {
        ...
        if (customItemError) {
          console.error("Error loading custom VR attempt items:", {
          ...
          customAttemptItemRows =
            (customItemData ?? []) as CustomVRAttemptItemRow[]
        }
      }

      const customProgressRows = buildCustomVRProgressRows(
        customAttemptRows,
        customAttemptItemRows
      )
  ...
  useEffect(() => {
    ...
    async function loadBenchmark() {
      ...
      if (error) {
        console.error("Error loading VR benchmark:", error)
        setBenchmarkData(null)
      } else {
        const benchmarkRows = (data ?? []) as VRBenchmarkRpcRow[]
          ...
          <SectionCard
            ...
            subtitle={
              benchmarkDifficulty === null
                ? "Your VR benchmark will appear here once there is a result with a difficulty level in the selected time period."
                : `This compares your recent ${benchmarkDifficultyLabel} VR work from ${benchmarkPeriodLabel} with anonymous YanBo attempts at the same difficulty level and time period.`
            }
          >
            {benchmarkDifficulty === null ? (
              <div style={emptyStateStyle}>
                Your VR benchmark will appear here once there is a result with
              ...
              <div style={emptyStateStyle}>
                Your VR benchmark will appear here once there is a recent result
                ...
                <div style={benchmarkMetricGridStyle}>
                  <div style={benchmarkMetricStyle}>
                    <div style={benchmarkMetricLabelStyle}>
                      Your recent {benchmarkDifficultyLabel} VR accuracy
                    </div>
                  ...
                  <div style={benchmarkMetricStyle}>
                    <div style={benchmarkMetricLabelStyle}>
                      YanBo average for {benchmarkDifficultyLabel} VR
                    </div>
                    <div style={benchmarkMetricValueStyle}>

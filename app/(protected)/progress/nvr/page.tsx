...
export default function NVRProgressPage() {
  const [progress, setProgress] = useState<NVRProgressRow[]>([])
  ...
  async function fetchProgress() {
    ...
    if (error) {
      console.error("Error loading NVR progress:", error)
      setLoading(false)
      return
    }
  ...
  const totalQuestionsPracticed = progress.reduce(
  ...
  if (loading) {
    return <p style={styles.message}>Loading NVR progress...</p>
  }

  return (
    <div style={styles.page}>
      <div style={styles.container}>
        <h1 style={styles.title}>🔷 Non-Verbal Reasoning Progress</h1>
        <p style={styles.subtitle}>
          Track non-verbal reasoning test performance over time.
        </p>
        ...
        <div style={styles.statsGrid}>
          ...
          <div style={styles.statCard}>
            <h3 style={styles.statLabel}>Questions Practised</h3>
            <p style={styles.statValue}>{totalQuestionsPracticed}</p>
          </div>
        ...
        {progress.length === 0 ? (
          <div style={styles.emptyCard}>
            <h2>No NVR progress yet</h2>
            <p>
              Complete a non-verbal reasoning test and your results will appear
              here.
            </p>
          </div>
        ) : (
                ...
                <tbody>
                  {progress.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td style={styles.td}>{row.total_questions}</td>
            ...
            <div style={styles.mobileList}>
              {progress.map((row) => (
                <div key={row.id} style={styles.attemptCard}>
                  ...
                  <p>
                    <strong>Total questions:</strong> {row.total_questions}
                  </p>
                  <p>
        ...
        <div style={styles.summaryCard}>
          ...
          <p>
            You answered <strong>{totalCorrectAnswers}</strong> NVR questions
